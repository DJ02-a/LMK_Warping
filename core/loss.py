...
class MyModelLoss(LossInterface):
    ...
    def get_loss_G(self, run_dict, valid=False):
        ...
        #     L_vgg = Loss.get_vgg_loss(F.interpolate(run_dict["fake_to_face"], (256,256), mode='bilinear'), F.interpolate(run_dict["to_face"], (256,256), mode='bilinear'))
        #     L_G += self.CONFIG['LOSS']['W_VGG'] * L_vgg
        #     self.loss_dict["L_vgg"] = round(L_vgg.item(), 4)
        
        if self.CONFIG['LOSS']['W_LPIPS']:
            L_lpips = Loss.get_lpips_loss(run_dict["fake_to_face"]*run_dict['to_mask'], run_dict["to_face"]*run_dict['to_mask'])
            L_G += self.CONFIG['LOSS']['W_LPIPS'] * L_lpips
            self.loss_dict["L_lpips"] = round(L_lpips.item(),4)
            
        if self.CONFIG['LOSS']['W_L1']:
            L_l1 = Loss.get_L1_loss(run_dict["fake_to_face"]*run_dict['to_mask'], run_dict["to_face"]*run_dict['to_mask'])
            L_G += self.CONFIG['LOSS']['W_L1'] * L_l1
            self.loss_dict["L_l1"] = round(L_l1.item(), 4)

        if self.CONFIG['LOSS']['W_BG']: 
            L_bg = Loss.get_L1_loss(run_dict["fake_to_face"]*run_dict['bg_mask'], run_dict["from_face"]*run_dict['bg_mask'])
            L_G += self.CONFIG['LOSS']['W_BG'] * L_bg
            self.loss_dict["L_bg"] = round(L_bg.item(), 4)
